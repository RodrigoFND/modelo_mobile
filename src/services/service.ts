import { RefreshTokenResponse } from "@/src/models/services/auth/auth.models";
...
import { TokenNotFoundError, SessionIdChangedError } from "@/src/errorsException";
...
const refreshToken = async ({
  ...
  // Adiciona um delay de 10 segundos
  await delay(10000);
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (isRefreshing) {
        console.warn("new Queue - accessToken");
